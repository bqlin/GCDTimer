...
@property (nonatomic, strong) BqTimer *countdown;
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event{
	...
	// 有结束点的定时器不需要被持有
	[BqTimer countdownWithSecond:self.countSecend countBlock:^(long remainSecond) {
...
- (IBAction)startTimer:(id)sender {
	...
	self.countdown = [BqTimer repeatWithBlock:^{
		NSLog(@"%02ld", i);
		i++;
	}];
}
